...
Public Sub AddRow(Values() As String)
	...
	If cAutomaticWidths = True Then
		...
		Dim ExtraWidth, width As Int
		ExtraWidth = 8dip + cLineWidth
		For i = 0 To NumberOfColumns - 1
...
Private Sub ShowRow(row As Int)
	...
	For I = 0 To lbls.Length - 1
		If (Header.GetView(I).Width > 1 ) Then 
			...
			SetPadding(lbls(I), 4dip, 2dip, 4dip, 2dip)
...
Private Sub CreateNewLabels As Label()
	...
	For i = 0 To NumberOfColumns - 1
		...
		L.Initialize("Cell")
		If MultiAlignments = False Then
			L.Gravity = cAlignment
		Else
			L.Gravity = cAlignments(i)
		End If
...
'	Dim Left = cLineWidth As Int
	...
	For i = 0 To NumberOfColumns - 1
		...
		Else
			L.Color = cHeaderColors(i)
			L.TextColor = cHeaderTextColors(i)
		End If
		
		L.Text = Values(i)
		L.Tag = i
'		w = Max(2dip, ColumnWidths(i) - 1dip)		' needed to make sure that the width has value >0
		w = Max(2dip, ColumnWidths(i) - cLineWidth)		' needed to make sure that the width has value >0
		Header.AddView(L, Left, 0, w, cRowHeight)
		If cAutomaticWidths = True Then
			Dim ExtraWidth As Int
			ExtraWidth = 8dip + cLineWidth
			If Values(i).Contains(CRLF) Then
				Dim str() As String
				Dim j As Int
				str = Regex.Split(CRLF, Values(i))
				HeaderWidths(i) = cvs.MeasureStringWidth(str(0), L.Typeface, cTextSize) + ExtraWidth
				For j = 1 To str.Length - 1
					HeaderWidths(i) = Max(HeaderWidths(i),cvs.MeasureStringWidth(str(j), L.Typeface, cTextSize)  + ExtraWidth)
				Next
			Else
				HeaderWidths(i) = cvs.MeasureStringWidth(Values(i), L.Typeface, cTextSize)  + ExtraWidth
			End If
...
Public Sub GetValue(Col As Int, row As Int) As String
	Dim values() As String 'stringa prima'
	values = Data.get(row)
	Return values(Col)
End Sub
...
Public Sub LoadTableFromCSV2(Dir As String, Filename As String, HeadersExist As Boolean, SeparatorChar As String, AutomaticWidths As Boolean)
	...
	Else
		If HeadersExist Then
			...
			For col = 0 To NumberOfColumns - 1
				HeaderWidths(col) = cvs.MeasureStringWidth(headers.get(col), Typeface.DEFAULT, cTextSize) + 8dip + cLineWidth
			Next
		End If
		For row = 0 To List1.Size - 1
			...
			For col = 0 To NumberOfColumns - 1
				DataWidths(col) = Max(DataWidths(col), cvs.MeasureStringWidth(strRow(col), Typeface.DEFAULT, cTextSize) + 8dip + cLineWidth)
			Next
		Next
		For col = 0 To NumberOfColumns - 1
...
Public Sub SetTypeFaces(TypeFaces() As Typeface)
	If TypeFaces.Length <> NumberOfColumns AND NumberOfColumns > 0 Then
